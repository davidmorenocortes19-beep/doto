...
export default function InventarioAdmin() {
  ...
  const eliminar = (item: InventarioItem) => {
    confirmar('Eliminar', `¿Eliminar el inventario de "${item.nombre}"?`, async () => {
      try {
        await axios.delete(`${API_URL}?id=${item.id_inventario}`, { timeout: 5000 });
        await cargar();
      } catch {
        notificar('Error', 'No se pudo eliminar el inventario');
      }
    });
  };
          ...
          {!verInhabilitados && (
            <>
              <TouchableOpacity
                ...
                onPress={() => router.push({ pathname: '/admin/editarInventario', params: { id: item.id_inventario } })}
              >
    ...
    <ImageBackground source={require('../../../assets/images/camiseta.png')} style={styles.background} resizeMode="cover">
      ...
      <View style={styles.container}>
        ...
        <View style={styles.accionesBar}>
          ...
          {!verInhabilitados && (
            <TouchableOpacity style={styles.btnAgregar} onPress={() => router.push('/admin/agregarInventario')}>
...
const styles = StyleSheet.create({
  ...
  btnAgregar: { backgroundColor: '#991B1B', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
  ...
  buscador: { marginHorizontal: 14, marginBottom: 8, backgroundColor: 'rgba(255,255,255,1.0)', color: '#0F172A', padding: 12, borderRadius: 8, borderWidth: 1.5, borderColor: '#991B1B', fontSize: 14 },

  lista: { paddingHorizontal: 14, paddingBottom: 24 },
  card: { backgroundColor: 'rgba(255,255,255,1.0)', borderWidth: 1.5, borderColor: '#991B1B', borderRadius: 12, padding: 14, marginBottom: 12 },
  ...
  btnEditar: { flex: 1, backgroundColor: '#991B1B', padding: 9, borderRadius: 8, alignItems: 'center' },
  btnEliminar: { backgroundColor: '#DC2626', padding: 9, borderRadius: 8, alignItems: 'center', paddingHorizontal: 12 },
