...
const API_REPORTES = 'http://192.168.137.9/doto/api';
...
export default function ProductosScreen() {
  ...
  const exportarPDF = async () => {
    try {
      await Linking.openURL(`${API_REPORTES}/reporteProductosPDF.php`);
    } catch {
  ...
  const exportarExcel = async () => {
    try {
      await Linking.openURL(`${API_REPORTES}/reporteProductosExcel.php`);
    } catch {
            ...
            <View style={styles.modalContenido}>
              ...
              <ScrollView>
                ...
                <View style={styles.chipsWrap}>
                  {(['Todos', 'Disponible', 'Agotado'] as FiltroEstado[]).map(opcion =>
                    renderChip(opcion, filtroEstado === opcion, () => setFiltroEstado(opcion))
                  )}
                </View>
                ...
                <View style={styles.chipsWrap}>
                  {RANGOS_PRECIO.map(r =>
                    renderChip(r.key, filtroPrecio === r.key, () => setFiltroPrecio(r.key), r.label)
                  )}
                </View>
              </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // Exportar — se mantienen colores semánticos (rojo PDF, verde Excel) pero más apagados
  ...
  estadoBadge:        { marginTop: 6, alignSelf: 'flex-start', backgroundColor: '#1E293B', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
