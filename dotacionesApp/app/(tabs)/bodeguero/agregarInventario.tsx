...
export default function AgregarInventarioAdmin() {
  ...
  const guardar = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await axios.post(API_INV, {
      ...
      if (res.data.mensaje) {
        setExitoMsg('✅ Inventario registrado correctamente');
        setTimeout(() => { setExitoMsg(''); router.replace('/admin/inventario'); }, 1500);
      } else {
        setErrorMsg(res.data.error || 'No se pudo registrar');
      }
    } catch (e: any) {
    ...
    <ImageBackground source={require('../../../assets/images/camiseta.png')} style={styles.background} resizeMode="cover">
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.replace('/bodeguero/inventario')} style={styles.btnVolver}>
            <Text style={styles.btnVolverTexto}>← Volver</Text>
          </TouchableOpacity>
          <Text style={styles.titulo}>Registrar Inventario</Text>
          <View style={{ width: 70 }} />
        </View>
        ...
        <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
          ...
          >
            {guardando
              ...
              : <Text style={styles.btnGuardarTexto}>💾 Registrar inventario</Text>
            }
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  errorBox: { backgroundColor: '#FEE2E2', borderWidth: 1.5, borderColor: '#9A3412', borderRadius: 10, padding: 14, marginBottom: 16 },
  errorTexto: { color: '#9A3412', fontWeight: '600', textAlign: 'center' },
  label: { color: '#0F172A', fontSize: 13, fontWeight: '600', marginBottom: 6, marginTop: 16 },
  input: { backgroundColor: 'rgba(255,255,255,1.0)', color: '#0F172A', padding: 14, borderRadius: 10, borderWidth: 1.5, borderColor: '#9A3412', fontSize: 15 },
  inputError: { borderColor: '#DC2626' },
  fieldHint: { color: '#DC2626', fontSize: 12, marginTop: 4 },
  fieldOk: { color: '#166534', fontSize: 12, marginTop: 4 },
  productoPreview: { backgroundColor: '#FEF2F2', borderWidth: 1, borderColor: '#9A3412', borderRadius: 8, padding: 10, marginTop: 6 },
  productoPreviewTexto: { color: '#0F172A', fontWeight: '600', fontSize: 13 },
  productoPreviewSub: { color: '#64748B', fontSize: 11, marginTop: 2 },
  btnGuardar: { backgroundColor: '#9A3412', padding: 15, borderRadius: 10, alignItems: 'center', marginTop: 30 },
