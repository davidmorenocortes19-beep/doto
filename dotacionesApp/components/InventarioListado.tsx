...
function estadoStock(item: InventarioItem): { texto: FiltroEstadoStock; badge: string } {
  if (Number(item.cantidad_actual) <= 0)                                         return { texto: 'Agotado',    badge: '#DC2626' };
  if (Number(item.cantidad_actual) <= Number(item.stock_minimo))                 return { texto: 'Stock bajo', badge: '#D97706' };
...
export default function InventarioListado({ volverA, formularioRuta }: InventarioListadoProps) {
  ...
  useEffect(() => { cargarInventario(); }, [verInhabilitados]);

  const resumen = useMemo(() => {
    const agotados  = inventario.filter(i => Number(i.cantidad_actual) <= 0).length;
    const stockBajo = inventario.filter(i => Number(i.cantidad_actual) > 0 && Number(i.cantidad_actual) <= Number(i.stock_minimo)).length;
    return { total: inventario.length, stockBajo, agotados };
  }, [inventario]);

  const tallasDisponibles = useMemo(() => ['Todas', ...Array.from(new Set(inventario.map(i => i.talla).filter(Boolean)))], [inventario]);
  const coloresDisponibles = useMemo(() => ['Todos', ...Array.from(new Set(inventario.map(i => i.color).filter(Boolean)))], [inventario]);
  ...
  const limpiarFiltros = () => { setFiltroEstado('Todos'); setFiltroTalla('Todas'); setFiltroColor('Todos'); setFiltroPrecio('Todos'); };
  ...
  const toggleInhabilitar = async (item: InventarioItem) => {
    try {
      ...
      Alert.alert('✅', `Inventario ${item.inhabilitado ? 'habilitado' : 'inhabilitado'} correctamente`);
    } catch {
      Alert.alert('Error', 'No se pudo actualizar el inventario');
    }
  };

  const editar   = (item: InventarioItem) => router.push({ pathname: formularioRuta as any, params: { id: item.id_inventario } });
  const eliminar = (item: InventarioItem) => {
    Alert.alert('Eliminar inventario', `¿Deseas eliminar el inventario de "${item.nombre}"?`, [
  ...
  const exportarPDF   = async () => { try { await Linking.openURL(`${API_BASE}/reporteInventarioPDF.php`); }   catch { Alert.alert('Error', 'No se pudo abrir el reporte PDF'); } };
  const exportarExcel = async () => { try { await Linking.openURL(`${API_BASE}/reporteInventarioExcel.php`); } catch { Alert.alert('Error', 'No se pudo abrir el reporte Excel'); } };
    ...
    return (
      <View style={styles.fila}>
        <View style={styles.infoBloque}>
          ...
          <Text style={styles.detalle}>ID: {item.id_producto_fk}</Text>
        ...
        <View style={styles.acciones}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              Alert.alert(
                item.inhabilitado ? 'Habilitar' : 'Inhabilitar',
                `¿Deseas ${item.inhabilitado ? 'habilitar' : 'inhabilitar'} el inventario de "${item.nombre}"?`,
                [
                  { text: 'Cancelar', style: 'cancel' },
          ...
          >
            <Text style={styles.btnTexto}>{item.inhabilitado ? '✅' : '🚫'}</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
      ...
      <View style={styles.container}>
        ...
        <View style={styles.accionesBar}>
          <TouchableOpacity
            ...
            onPress={() => { setVerInhabilitados(!verInhabilitados); setInventario([]); }}
          >
        ...
        {!cargando && filtrados.length === 0 && error === '' && (
          <Text style={styles.sinResultados}>{verInhabilitados ? 'No hay inventario inhabilitado' : 'No se encontraron registros'}</Text>
        )}
            ...
            <View style={styles.modalContenido}>
              ...
              <ScrollView>
                ...
                <View style={styles.chipsWrap}>
                  {(['Todos', 'Disponible', 'Stock bajo', 'Agotado'] as FiltroEstadoStock[]).map(op => renderChip(op, filtroEstado === op, () => setFiltroEstado(op)))}
                </View>
...
const styles = StyleSheet.create({
  ...
  btnEliminar: { backgroundColor: '#DC2626', padding: 10, borderRadius: 8, alignItems: 'center', paddingHorizontal: 12 },
