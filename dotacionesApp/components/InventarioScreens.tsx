...
function rutasPorRol(rol: RolInventario): RutasInventario {
  return rol === 'admin'
    ? {
      ...
      formulario: '/admin/inventario',
      registros: '/admin/inventario_registros',
    }
    : {
      panel: '/bodeguero/panel_bodeguero',
      formulario: '/bodeguero/inventario',
      registros: '/bodeguero/inventario_registros',
    };
}

function estadoStock(item: InventarioItem): { texto: string; color: string } {
  if (Number(item.cantidad_actual) <= 0) {
    return { texto: 'Agotado', color: '#333333' };
  }
...
export function InventarioRegistrosScreen({ rol }: { rol: RolInventario }) {
  ...
  const editar = (item: InventarioItem) => {
    router.push({
      pathname: rutas.formulario as any,
...
const styles = StyleSheet.create({
  ...
  formulario: { margin: 12, backgroundColor: '#F8F9FA', borderRadius: 10, padding: 14, borderWidth: 1, borderColor: '#333333' },
  formTitulo: { color: '#333333', fontWeight: 'bold', fontSize: 16, marginBottom: 10 },
  input: { backgroundColor: '#F8F9FA', padding: 12, borderRadius: 8, marginBottom: 8, borderWidth: 1, borderColor: '#333333', fontSize: 14, color: '#333333' },
  ...
  mensaje: { color: '#333333', textAlign: 'center', marginHorizontal: 12, marginBottom: 8, fontSize: 13 },
