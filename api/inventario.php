...
switch ($metodo) {
    ...
    case 'GET':
        ...
        if (!empty($_GET['inhabilitados'])) {
            responder(200, Inventario::listarInhabilitados());
        }
        if (isset($_GET['id'])) {
    ...
    case 'PUT':
        ...
        if (isset($body['inhabilitado'])) {
            $resultado = Inventario::toggleInhabilitado(
                (int) $body['id_inventario'],
                (bool) $body['inhabilitado']
            );
            if ($resultado) {
                responder(200, ['mensaje' => $body['inhabilitado'] ? 'Inventario inhabilitado' : 'Inventario habilitado']);
            } else {
...
function validarCampos(?array $body): ?string {
    ...
    if ((int) $body['id_producto_fk'] <= 0) return 'El producto debe ser válido';
    if ((int) $body['cantidad_actual'] < 0) return 'La cantidad actual no puede ser negativa';
    if ((int) $body['stock_minimo'] < 0)    return 'El stock mínimo no puede ser negativo';
    return null;
}
